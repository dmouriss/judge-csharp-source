            ...
            switch (nodetype)
            {
                ...
                case "test-case":
                    if (node.SelectNodes("assertions").Count == 0) /* runtime error */
                    {
                        ...
                        OutputChannel.WriteLine(JSONGenerator.CloseTestcase(false));
                    }
                    else
                    {
                        bool? accepted = null;
                        foreach (XmlNode assertion in node.SelectSingleNode("assertions"))
                        {
                            ...
                            else if (resultmap[testresult] != Status.CORRECT)
                            {
                                accepted = false;
                            }
                        }
                        OutputChannel.WriteLine(JSONGenerator.CloseTestcase(accepted));
                    }
